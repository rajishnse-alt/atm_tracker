...
def _calculate_26_11_levels(spot, symbol):
    ...
    Caches data and refreshes every 3 minutes to avoid rate limiting.
    """
    now_ist = datetime.now(IST)
...
def render_symbol(access_token, sym, vix_info, now_ist):
    ...
    if st.session_state.get(debug_key, False):
        with st.expander("🔍 **DEBUG LOG** - 26.11 Levels (NSE Data)", expanded=True):
            ...
            # Get NSE quote data (symbol-specific key)
            nse_debug = st.session_state.get(f"api_response_{sym}", {})
